...
[https://github.com/coledefeo/Defeo_Cole_201_A06](https://github.com/coledefeo/Defeo_Cole_201_A06)
...
for(i in names(player_info)){
  player_ppg[[i]] <- player_info[[i]][[4]] / player_info[[i]][[3]]
}
...
```{r}
player_vec <- unlist(lapply(player_ppg, function(x) round(x,2)))
...
df_round <- apply(mat_rand, MARGIN = 1:2, round_num)
df_round
...
```{r}
max_temps_num <- lapply(max_temps, as.numeric) #converting values to numeric
max_temps_num <- as.data.frame(max_temps_num)

mean_temps <- apply(max_temps_num, 2 , mean, na.rm = TRUE) # apply() to find mean temps in each column
...
```{r}
temps_co_num <- as.data.frame(apply(temps_co, 1:2,  as.numeric)) # converting temps_co to numeric and back to data frame
temps_co_num <- temps_co_num[, -c(1,2)] #removing state and year columns

all_median_temps <- apply(temps_co_num,1,  median, na.rm = TRUE) # just median temps


years <- 1895:2020  # sequence to correspond years with median temps

med_temps_df <- data.frame( # creating a data frame with median temperatures and years
  year = years,
  median_temp = all_median_temps
)

# Finds (in order) max med temp and its corresponding year and min med temps and its corresponding year
max_med_temp <- med_temps_df$median_temp[which.max(med_temps_df$median_temp)] 
max_year <- med_temps_df$year[which.max(med_temps_df$median_temp)]
min_med_temp <- med_temps_df$median_temp[which.min(med_temps_df$median_temp)] 
min_year <- med_temps_df$year[which.min(med_temps_df$median_temp)]
...
digit_sum <- function(x) {
  if(x < 10){ # base case
    ...
    last_digit <-x %% 10 # last digit of number to add to function
    trunc_num <- x %/% 10 # the remaining number once the last digit is removed
    return(last_digit + digit_sum(trunc_num))
  }
}

print(digit_sum(935068))
```
