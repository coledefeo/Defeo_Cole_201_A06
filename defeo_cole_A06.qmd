...
mean_temps <- apply(max_temps_num, MARGIN = 2 ,FUN = mean, na.rm = TRUE) # apply() to find mean temps in each column
...
```{r}
temps_co_num <- lapply(temps_co, as.numeric)
temps_co_num <- as.data.frame(temps_co_num) # converting temps_co to numeric and back to data frame

all_median_temps <- apply(temps_co_num, MARGIN = 1, FUN = median, na.rm = TRUE) # just median temps
